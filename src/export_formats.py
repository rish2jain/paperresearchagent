...
common themes, """
        ...
        + str(len(gaps))
        + """ 
research gaps. Generated automatically using Agentic Researcher.
\end{abstract}
...
def generate_word_document(
    ...
    # Add metadata
    doc.add_paragraph(f"Generated by Agentic Researcher")
    ...
    abstract_text = (
        ...
        f"research gaps. Generated automatically using Agentic Researcher."
    )
...
def generate_pdf_document(
    ...
    # Add metadata
    meta_text = (
        f"Generated by Agentic Researcher<br/>"
...
def generate_zotero_ris_export(papers: List[Dict[str, Any]]) -> str:
    """
    Generate RIS format export for Zotero
    ...
    for paper in papers:
        ...
        if "arxiv" in source:
            ...
            entry_type = "JOUR"
        elif "conference" in source or "proceedings" in source:
            entry_type = "CONF"
        else:
            entry_type = "JOUR"  # Default
        
        entry_lines.append(f"TY  - {entry_type}")
        ...
        # Abstract
        if paper.get("abstract"):
            # RIS abstracts can be multiline
        ...
        if paper.get("journal") or paper.get("venue"):
            ...
            entry_lines.append(f"JO  - {venue}")
        
        # Source database
        if paper.get("source"):
            entry_lines.append(f"DB  - {paper['source']}")
        
        # Paper ID
        paper_id = paper.get("id", "")
        if paper_id:
            entry_lines.append(f"ID  - {paper_id}")
...
def generate_mendeley_csv_export(papers: List[Dict[str, Any]]) -> str:
    """
    ...
    # Mendeley CSV header
    fieldnames = [
        "Title", "Authors", "Year", "Publication Title", "Volume",
        "Issue", "Pages", "DOI", "URL", "Abstract", "Type", "Source"
    ]
    ...
    for paper in papers:
        ...
        row = {
            ...
            "Publication Title": paper.get("journal") or paper.get("venue", ""),
...
def generate_interactive_html_report(
    ...
    html_parts.append("""        <footer>
            <p>Generated by Agentic Researcher | Powered by NVIDIA NIMs</p>
        </footer>""")
